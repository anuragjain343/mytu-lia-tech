...
class Service extends CommonService{
    ...
    function inspirations_post(){
      $offset = $this->post('offset'); 
      $limit = $this->post('limit');
        if(!isset($offset) || !isset($limit)){
            $offset = 0; $limit= 50;
        }
    $result = $this->service_model->inspirations_data($offset,$limit);
    if($result){
            $response = array('status' => SUCCESS, 'message' => ResponseMessages::getStatusCodeMessage(126), 'inspirations' => $result);
        }else{
            $response = array('status' => FAIL, 'message' => ResponseMessages::getStatusCodeMessage(131), 'inspirations' => $result);
        }
         $this->response($response);
    }
