...
class Service_model extends CI_Model {
	...
	*/
	...
	function userInfo($where){
            ...
            //adding alias for user id since there may be case of ambiguious column
            if (array_key_exists("id",$where)){
                $where['u.id'] = $where['id'];
                unset($where['id']); //unset old key
            }
            $req = $this->db->select('u.id,u.userName,u.fullName,u.latitude,u.longitude,u.email,u.userType,u.countryCode,u.contactNumber,u.address,u.authToken,u.status,u.createdOn,u.image, COALESCE(um.description, "") as description, COALESCE(um.currency_code, "") as currency_code, COALESCE(um.currency_symbol, "") as currency_symbol,  COALESCE(um.price, "") as price, COALESCE(att.attachment_name, "") as originalImage')->join(USER_META. ' as um', "u.id = um.user_id","left")->join(ATTACHMENTS. ' as att', "u.id = att.reference_id AND att.reference_table = 'users' AND att.image_size = 'default'","left")->where($where)->get(USERS.' as u');
            $returnData = array();
            if($req->num_rows())
            {
                $result = $req->row();
                ...
                */
                if (filter_var($result->image, FILTER_VALIDATE_URL)) {
                    //user profile image
                    $result->thumbImage = make_user_img_url($result->image); //make url from image name
                }else{
                    //user original image
                    $result->thumbImage = make_user_img_url($result->originalImage); //make original image as thumb
                }
                
                /* Commented with reference to above changes by Manish
   ...
   function inspirations_data($offset,$limit){
   
   	$defaultImg = base_url().USER_DEFAULT_AVATAR;
   	$blog_image = base_url().BLOG_CONTENT;
    $this->db->select('b.blogId,b.title,b.description,b.createdOn,b.status,(case 
            when( atc.attachment_name = "" OR atc.attachment_name IS NULL) 
            THEN "'.$defaultImg.'"
            ELSE
            concat("'.$blog_image.'",atc.attachment_name)	END ) as BlogImage');
    $this->db->from(BLOG.' as b');
    $this->db->join(ATTACHMENTS. ' as atc', "atc.reference_id = b.blogId AND atc.reference_table ='blogs' AND atc.image_size = 'medium'",'left'); 
