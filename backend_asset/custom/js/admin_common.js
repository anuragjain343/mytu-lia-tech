...
//                            });
 ...
 var sendNOtification = function (ctrl, method, id) {
        $.ajax({
            url: base_url + ctrl + "/" + method,
            type: 'POST',
            data: {'id': id},
    ...
    var deleteFn = function (table, field, id, ctrl, method) {
     ...
     var deleteFunc = function (table, field, id, ctrl, method) {
       ...
       var deleteFuncPost = function (table, field, id, ctrl, method) {
        if(typeof method == "undefined" || method==""){
            ...
            callback: function (result) {
                ...
                if (result) {
                    ...
                    $.ajax({
                        ...
                        url: url,
                        dataType: "json",
                        data: {id: id, is_delete: field, table: table},
                        success: function (data) {
                           hide_loader(); 
                            if (data.status == 1) {
                                toastr.success(data.message);
        ...
        var deleteFuncBlog = function (table, field, id, ctrl, method) {
        if(typeof method == "undefined" || method==""){
            method = "admin/blogs";
        }
        else{
...
$(function () {
  ...
  $('body').on('click', ".add_blog", function (event) { 
    ...
    table = $('#blog_list').DataTable({ 
        ...
        // Load data for the table's content from an Ajax source
        "ajax": {
            "url": base_url+"admin/imaginations/get_blog_list_ajax",
            "type": "POST",
            "dataType": "json",
            "dataSrc": function (jsonData) {
                return jsonData.data;
...
function addfeatursUser(id){
 var uid  = $('.'+id);
   $.ajax({
            type: "POST",
            url: base_url+'admin/users/FeaturesUser',
            data : {userId : id},
            cache: false,
            dataType: "JSON", 
            beforeSend: function () { 
            ...
            if(data.status == '1'){
               ...
               uid.css("color","green");
               uid.attr('title', 'Removed featured vendor');
               $("#colorChange").attr('title', 'Removed featured vendor');
            }

            if(data.status == '2'){
                toastr.success(data.message);
               $("#colorChange").css("color","gray");
                uid.css("color","gray");
                uid.attr('title', 'Add as featured vendor');
              $("#colorChange").attr('title', 'Add as featured vendor');
   ...
   $(document).on('submit', "#sendFormAjax", function (event) {
     ...
     $(document).on('submit', "#sendAllFormAjax", function (event) {
