...
class Post extends CommonService{
    ...
    function createPost_post(){
        ...
        if($post_id){  
            ...
            if(!empty($all_users['result']) && !empty($all_tokens['result'])){
                ...
                //temporary fix for bulk push notifications - BY manish(27-09-2018) 
                
                    $notif_msg['body'] = $body_save; //replace body text with placeholder text
                    foreach($all_users['result'] as $u){
                        $registrationIds = array($u->deviceToken);
    ...
    function createPostNotification_post($postId,$userId,$eventName,$userName,$cat_id){
        ...
        if(!empty($all_users['result']) && !empty($all_tokens['result'])){
            ...
            $registrationIds = explode(",", $all_tokens['result'][0]->deviceToken); $title = "New post created";
            //body to be sent with current notification
            $body_send = $userName.' posted event '.$eventName; 
            //body to be saved in DB
            $body_save = '[UNAME] posted event [ENAME]';
            $notif_type = 'post_create';

            $notif_msg = $this->send_push_notification($registrationIds, $title, $body_send, $postId, $notif_type);
            if($notif_msg){
                $notif_msg['body'] = $body_save; //replace body text with placeholder text
                foreach($all_users['result'] as $u){
                    //save notification
                    $insertdata = array('notification_by'=>$userId, 'notification_for'=>$u->user_id, 'notification_message'=>json_encode($notif_msg), 'notification_type'=>$notif_type, 'reference_id'=>$notif_msg['reference_id'] ,'created_on'=>datetime());
                    $this->notification_model->save_notification(NOTIFICATIONS, $insertdata);
                }
            }
        }
    ...
    public function updatePost_post(){
        ...
        if(!empty($category)){
        $update_data_map['category_id'] =$category;
        }
         $where_map = array('post_id'=>$post_id);
        if(!empty($category)){
             $is_exits_category = $this->common_model->is_data_exists('doing_event', array('post_id'=>$post_id));
         if($is_exits_category){
          $response = array('status' => FAIL, 'message' => 'Allready one vendor doing this event');
             $this->response($response);  
         }
        $is_updated_map = $this->common_model->updateFields('post_category_mapping',$update_data_map, $where_map);
        }
        if($is_updated){
