...
class Posts extends Common_Controller {
    ...
    public function get_post_list_ajax(){
        ...
        foreach ($list as $post) { 
            ...
            $clk_event = "deleteFuncPost('".POSTS."' ,'1','$post->id ','admin/posts/','deletePost')";
            $action .= '<a href="javascript:void(0)"  onclick="'.$clk_event.'" class="on-default edit-row table_action" title="Delete post">'.DELETE_ICON.'</a>';
            $row[] = $action;
            $data[] = $row;
            $_POST['draw']='';
        }
    ...
    public function deletePost(){
        ...
        $where  = array('id'=>$id);
        $st     =    $_POST['is_delete'];
        $data   = array('is_delete'=>$st);
        $table  =    $_POST['table'];
        $upd = $this->common_model->updateFields($table, $data, $where);
        if($upd==TRUE){
            $output = array(
              "status" => 1,
