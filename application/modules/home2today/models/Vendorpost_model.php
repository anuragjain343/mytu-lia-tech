...
class Vendorpost_model extends CI_Model {
    ...
    function get_post_list($where, $user_id, $limit, $offset=0,$latitude,$longitude){
        ...
        $this->db->select('doi.*,p.*,round(( 6371 * acos( cos( radians("'.$latitude.'") ) * cos( radians(  usr.latitude  ) ) * cos( radians( usr.longitude ) - radians("'.$longitude.'") ) + sin( radians("'.$latitude.'") ) * sin( radians( usr.latitude ) ) ) ),1) AS distance_in_mi, eve.id as eventId, eve.event_name, usr.fullName, usr.email, usr.image as user_image, cat.id as catId, cat.name as category_name, COUNT(doi.id) as interested_count, '.$img_field3);
