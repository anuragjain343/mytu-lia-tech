...
class Users extends FrontCommon_Controller {
    ...
    { 
        ...
        { 
            ...
            $set = array('fullName','email', 'contactNumber','address','latitude','longitude');
    ...
    function vendorList()
    {
        ...
        $select ='*';
        $whr = array('id'=>$this->session->userdata('id'));
        $data['check_lat_log'] = $this->common_model->customGetId($select,$whr,USERS);
        $data['check_lat_log']->latitude;
        $data['check_lat_log']->longitude;
     // $data['cat_id'] = decoding($this->uri->segment('4'));
        if(!empty($_GET['id']) && !empty($_GET['location']) && $_GET['lat'] && $_GET['long']){
        $data['cat_id']         =   decoding($_GET['id']);
        $where                  =   array('category_id'=>$data['cat_id']);
        ...
        if(empty($_GET['lat']) &&  empty($_GET['long']) && !empty($_GET['id'])){
        $data['cat_id']         =   decoding($_GET['id']);
        $data['lat']            =    $data['check_lat_log']->latitude;
        $data['long']           =    $data['check_lat_log']->longitude;

        }
        if(empty($_GET['id']) && !empty( $_GET['lat']) && !empty( $_GET['long'])){
        $data['lat']            =    $_GET['lat']; 
        $data['long']           =    $_GET['long'];
        $data['location']       =    $_GET['location'];
        }
...
function vendor_cat_list()
    { 
        ...
        $fbUserLocLat = $this->input->post('fbUserLocLat');
        $fbUserLocLong = $this->input->post('fbUserLocLong');
        $select ='*';
        $whr = array('id'=>$this->session->userdata('id'));
        $data['check_lat_log'] = $this->common_model->customGetId($select,$whr,USERS);
       
        if(!empty($fbUserLocLat) && !empty($fbUserLocLong)){
        $lat = $fbUserLocLat;
        $long = $fbUserLocLong;
        }else{
          $lat  = $data['check_lat_log']->latitude;
          $long = $data['check_lat_log']->longitude;
        }
        $category_id =  decoding($this->input->post('category'));
        $cat = decoding($this->input->post('id'));

        $lt  = $this->input->post('letitu');
        $lng = $this->input->post('longitu');
        
        if(!empty($lt) && !empty($lng)){
        $lat  = $lt;
        $long = $lng;
        }
        ...
        //print_r($category_id);  echo "string";
       // pr($cat);
        
        $this->load->library('Ajax_pagination');
        $user_id = !empty($this->session->userdata['id'])?$this->session->userdata['id']:'' ;

        $total = $this->common_model->search_user_list_count('vendor',$category_id,$cat,$lat,$long,$check_status=true,$user_id); 
       ...
       // echo $user_id;
        $data['vendor_list']     = $this->common_model->search_user_list('vendor', $category_id,$cat,$lat,$long,$limit, $offset, $check_status=true,$user_id,$catid); 
     ...
     //   pr( $data['pagination']);
      ...
      // $data['urllink'] = "http://localhost/dev_tulia/home/users/vendorList/?id=";
       $this->load->view('vendor_cat_list',$data);
    }
    ...
    function notification_list()
    {
        ...
        foreach($all_notif as $k=>$v)
        {
            ...
            if($v->notification_by==1){
               $v->user_image =  base_url().ADMIN_IMAGE_PATH.$v->image;  
            }else{
                $v->user_image = make_user_img_url($v->image); 
            }
            //make url from image name
    ...
    /*
     * Created by Manish for album changes
     * Do not use this method
     */
    public function re_upload(){
        
        $folder = 'user_album';
        $where = array('reference_table'=> 'albums', 'image_size'=>'default');
        $img_path = FCPATH.'uploads/user_album/';
        $sql = $this->db->select('*')->where($where)->limit(500)->get('attachments'); 
        //$this->db->limit(1);
        $att = $sql->result();
        //pr($att);
        $this->load->model('imageCrop_model');
        if(!empty($att)){
            foreach($att as $v){
                
                $source_img = $img_path.$v->attachment_name;
                
                $is_where = array('attachment_name'=>$v->attachment_name);
                $is_exist = $this->common_model->is_data_exists('album_attachments', $is_where);
                    
                if(file_exists($source_img) && !$is_exist){
                    $image_data['full_path'] = $source_img;
                    $image_data['file_name'] = $v->attachment_name;
                    
                    $source_image_arr = getimagesize($source_img);
                    $source_image_width = $source_image_arr[0];
                    $source_image_height = $source_image_arr[1];
                    
                    $image_data["image_width"] = $source_image_width;
                    $image_data["image_height"] = $source_image_height;

                    $resized_img = $this->imageCrop_model->resize_n_crop($image_data, $folder);
                    echo $v->attachment_name . ' ----- ' .$resized_img; echo '<br>';
                    
                    $is_where = array('attachment_name'=>$resized_img);
                    $is_exist = $this->common_model->is_data_exists('album_attachments', $is_where);
                    
                    if(!empty($resized_img) && !$is_exist){
                        
                        echo '>>>  '.$v->attachment_name . ' ----- ' .$resized_img; echo '<br>';
                        $dataInsert = array(
                            'album_id'=> $v->reference_id, 
                            'attachment_name'=>$resized_img, 
                            'created_on' => datetime() 
                            );
                        $this->common_model->insertData('album_attachments', $dataInsert);
                    }
                }
            }
        }
