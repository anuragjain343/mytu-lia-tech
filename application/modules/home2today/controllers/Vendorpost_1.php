...
*/
        ...
        $this->load->model('image_model');
            ...
            if(!empty($_POST['album_images'])){
                ...
                for($i = 0; $i < $filesCount; $i++){
                    ...
                    $res = $this->image_model->canvasImageUpload($_POST['album_images'][$i],'user_album',rand());
                  
                    //file_put_contents($realImagePath,file_get_contents($_POST['album_images'][$i]));
                    //upload each image and insert d$res;
                    $attr_data['reference_id']    = $album_id;
                    $attr_data['reference_table'] = 'albums';
                    $attr_data['image_size']      = 'large';
                    $attr_data['attachment_type'] = 'image';
                    $attr_data['attachment_name'] = $res['uploadfile'];
                     $this->db->insert(ATTACHMENTS, $attr_data);
    ...
    function deleteImage(){
        ...
        $this->common_model->deleteData(ATTACHMENTS, array('id'=>$album_id));
        //lq();
        $response = array('status' => 1, 'message' => 'Deleted Successfully', 'url' => base_url('home/users/vendorMedia')); //success msg
        echo json_encode($response);
    ...
    function update_media(){
        ...
        $current_user_id = $this->session->userdata['id']; //user id
            ...
            for($i = 0; $i < $filesCount; $i++){
                    $rand  = rand().'.png';
                    $realImagePath = FCPATH.'uploads/user_album/mindiii-'.$rand;
                    $res = $this->image_model->canvasImageUpload($_POST['album_images'][$i],'user_album',rand());
                    ...
                    $attr_data['attachment_type'] = 'image';
                    $attr_data['attachment_name'] = $res['uploadfile'];
                    //$attr_data['user_id'] = $current_user_id;
                    //$attr_data['updated_on'] = datetime();
                     //$this->db->insert(ATTACHMENTS, $attr_data);
            ...
            $this->common_model->updateFields(ALBUMS, $update_data, array('id'=>$album_id));
