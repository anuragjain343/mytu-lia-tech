...
class Vendorpost_model extends CI_Model {
   ...
   /* function get_post_list($where, $user_id, $limit, $offset=0){
    ...
    function get_post_list($where, $user_id, $limit, $offset=0,$latitude,$longitude){
        ...
        $this->db->select('doi.*,p.*,round(( 6371 * acos( cos( radians("'.$latitude.'") ) * cos( radians(  p.latitude  ) ) * cos( radians( p.longitude ) - radians("'.$longitude.'") ) + sin( radians("'.$latitude.'") ) * sin( radians( p.latitude ) ) ) ),1) AS distance_in_mi, eve.id as eventId, eve.event_name, usr.fullName, usr.email, usr.image as user_image, cat.id as catId, cat.name as category_name, COUNT(doi.id) as interested_count, '.$img_field3);
        $this->db->from(POSTS .' as p');
        $this->db->join(EVENT_TYPE. ' as eve', "p.event_type = eve.id","left"); //to get event type details
        $this->db->join(DOING_EVENT. ' as doi', "p.id = doi.post_id","left"); //to get doing event count
        $this->db->join(POST_CAT_MAPPING. ' as map', "p.id = map.post_id","left"); //to get post category
