...
class Posts extends FrontCommon_Controller {
    ...
    { 
        ...
        $is_exist = $this->common_model->is_data_exists(POSTS, array('id'=>$post_id,'status'=>1,'is_delete'=>'0'));
        if(!$is_exist){
            $data['front_scripts'] = $data['front_styles'] = '';
   ...
   // update post view load here
    function editPost($post_id)
    {
        ...
        $data['event_type']    = $this->home_model->getAllData(EVENT_TYPE);
        $data['post_detail'] = $this->post_model->get_post_detail(array('p.id'=>$post_id));
        $data['category_exits'] = $this->common_model->is_data_exists('doing_event', array('post_id'=>$post_id));
 ...
 function updatePost()
    {
        ...
        $is_updated = $this->common_model->updateFields(POSTS, $update_data, $where);
        ...
        if(!empty($category)){
         ...
         if($is_exits_category){
          $response = array('status' => FAIL, 'message' => 'Cannnot edit allready one vendor doing this event');
            echo json_encode($response); die;  
         }
