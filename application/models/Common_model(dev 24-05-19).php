...
class Common_model extends CI_Model {
    ...
    function customGetId($select,$where,$table){
        $this->db->select($select);
        $this->db->where($where);
        $sql = $this->db->get($table);
        return $sql->row();
    ...
    function getMultipleData1($select,$where,$table){
        $this->db->select($select);
        $this->db->where($where);
        $sql = $this->db->get($table);
        return $sql->result();
    ...
    function user_details($user_id, $check_status=true){
        ...
        $this->db->select('u.*, COALESCE(um.description, "") as description, COALESCE(um.price, "") as price, COALESCE(um.currency_code, "") as currency_code, COALESCE(um.currency_symbol, "") as currency_symbol, COALESCE(AVG(r.rating), "") as rating, COUNT(r.id) as total_rating, COALESCE(att.attachment_name, "") as originalImage');
        ...
        $this->db->join(ATTACHMENTS. ' as att', "u.id = att.reference_id AND att.reference_table = 'users' AND att.image_size = 'default'","left"); //to get user attachement details
        ...
        if(!empty($u_detail)){
            ...
            /* App team needed original image as user thumb image so have made some changes here
             * First we have put a JOIN with attachment table to get default(original) image
             * First we check if image from FB(socail platform)
             * If it is from FB then then return it as it is
             * If not then replace the same key with original image
             * Changes done by Manish on 02-10-2018
             */
            if (filter_var($u_detail->image, FILTER_VALIDATE_URL)) {
                //user profile image
                $u_detail->thumbImage = make_user_img_url($u_detail->image); //make url from image name
            }else{
                //user original image
                $u_detail->thumbImage = make_user_img_url($u_detail->originalImage); //make original image as thumb
            }
   ...
   /* function search_user_list_count($user_type, $category_id, $search_str,$check_status=true,$user_id =''){
   ...
   //filter user list by name
   /* function search_user_list($user_type, $category_id, $search_str,  $limit, $offset=3, $check_status=true,$user_id =''){
    ...
    function search_user_list1($user_type, $category_id, $search_str,  $limit, $offset=3, $check_status=true,$user_id =''){
  ...
  /*  function search_user_list_count($user_type, $category_id, $search_str,$check_status=true,$user_id =''){
       if(!empty($user_id)){
                ...
                $userlt = $this->customGet($fld,$wh,USERS);
                
                $latitude= $userlt['latitude'];
                $longitude = $userlt['longitude'];
              
            $this->db->select('u.*, COALESCE(um.description, "") as description, COALESCE(um.price, "") as price, COALESCE(AVG(r.rating), "") as rating, COALESCE(um.currency_code, "") as currency_code, COALESCE(um.currency_symbol, "") as currency_symbol, COALESCE(cat.name, "") as category_name,round(( 6371 * acos( cos( radians("'.$latitude.'") ) * cos( radians(  u.latitude  ) ) * cos( radians( u.longitude ) - radians("'.$longitude.'") ) + sin( radians("'.$latitude.'") ) * sin( radians( u.latitude ) ) ) ),1) AS distance_in_mi'); 
            }else{
            $this->db->select('u.*, COALESCE(um.description, "") as description, COALESCE(um.price, "") as price, COALESCE(AVG(r.rating), "") as rating, COALESCE(um.currency_code, "") as currency_code, COALESCE(um.currency_symbol, "") as currency_symbol, COALESCE(cat.name, "") as category_name');            
            }
...
/*     function search_user_list_count($user_type, $category_id, $search_str,$check_status=true,$user_id =''){
    ...
    function search_user_list_count($user_type, $category_id,$cat,$lat,$long,$check_status=true,$user_id =''){
       ...
       if(!empty($user_id)){
            ...
            $this->db->select('u.*, COALESCE(um.description, "") as description, COALESCE(um.price, "") as price, COALESCE(AVG(r.rating), "") as rating, COALESCE(um.currency_code, "") as currency_code, COALESCE(um.currency_symbol, "") as currency_symbol, COALESCE(cat.name, "") as category_name,round(( 6371 * acos( cos( radians("'.$latitude.'") ) * cos( radians(  u.latitude  ) ) * cos( radians( u.longitude ) - radians("'.$longitude.'") ) + sin( radians("'.$latitude.'") ) * sin( radians( u.latitude ) ) ) ),1) AS distance_in_mi'); 
               ...
               $this->db->select('u.*, COALESCE(um.description, "") as description, COALESCE(um.price, "") as price, COALESCE(AVG(r.rating), "") as rating, COALESCE(um.currency_code, "") as currency_code, COALESCE(um.currency_symbol, "") as currency_symbol, COALESCE(cat.name, "") as category_name,round(( 6371 * acos( cos( radians("'.$lat.'") ) * cos( radians(  u.latitude  ) ) * cos( radians( u.longitude ) - radians("'.$long.'") ) + sin( radians("'.$lat.'") ) * sin( radians( u.latitude ) ) ) ),1) AS distance_in_mi'); 
        ...
        switch ($user_type) {
            ...
            case "vendor":
                ...
                if(!empty($category_id)){
                   ...
                   if(!empty($cat)){
                    if($cat=='Select Vendor'){ 
                        $cat=" ";
                    }else{
                        $where['map.category_id']= $cat; 
                    }
                }
        ...
        $this->db->group_by('u.id');
        $this->db->order_by('u.id', 'DESC');
         if((!empty($latitude) && !empty($longitude)) OR (!empty($lat) && !empty($long))){
        $this->db->having(array('distance_in_mi <='=>'100')); 
        }
        $result = $this->db->get();
        //lq();
       ...
       //filter user list by name
   function search_user_list($user_type, $category_id,$cat,$lat,$long,$limit, $offset=3, $check_status=true,$user_id =''){
   ...
   //related vendor according to cat_id
    function related_vendor($user_type,$vendor_id,$category_id,$check_status=true,$user_id=''){
    ...
    function get_user_album_list($user_id, $limit, $offset=0){
        ...
        $alb_img_path = base_url().ALBUM_IMAGE_PATH.'thumb/';
        ...
        if(!empty($res)){
            $fields = 'id, '.$img_field;
            foreach($res as $v){
                $alb_where = array('album_id'=> $v->id);
                $album = $this->getAllwhere(ALBUM_ATTACHMENTS, $alb_where, 'id', 'ASC', $fields, 5); 
    ...
    function get_user_album_by_id($album_id){
        ...
        if(!empty($res)){
            $fields = 'id, '.$img_field;
            $alb_where = array('album_id'=> $album_id);
            $album = $this->getAllwhere(ALBUM_ATTACHMENTS, $alb_where, 'id', 'ASC', $fields, 5);
            //pr($album);
    ...
    function get_doing_user_list($post_id, $limit, $offset=0, $check_status=true){
     ...
     function user_list1($user_type,$category_id, $limit,$offset=0, $check_status=true,$latitude,$longitude){
        ...
        $this->db->select('u.*, COALESCE(um.description, "") as description, COALESCE(um.price, "") as price, COALESCE(AVG(r.rating), "") as rating, COALESCE(um.currency_code, "") as currency_code, COALESCE(um.currency_symbol, "") as currency_symbol,round(( 6371 * acos( cos( radians("'.$latitude.'") ) * cos( radians(  u.latitude  ) ) * cos( radians( u.longitude ) - radians("'.$longitude.'") ) + sin( radians("'.$latitude.'") ) * sin( radians( u.latitude ) ) ) ),1) AS distance_in_mi');
        }
    ...
    //delete all album attachments from directory and table
    function delete_album_attachments($album_id){
        
        $del_where = array('album_id'=>$album_id);
        $alb_folder = ALBUM_IMAGE_PATH;
        
        $att_data = $this->getAllwhere(ALBUM_ATTACHMENTS, $del_where);  //get all attachments of reference
        if(!empty($att_data['result'])){
            foreach($att_data['result'] as $row){
                
                $this->imageCrop_model->delete_image($alb_folder, $row->attachment_name);
            }
        }
        
        $this->deleteData(ALBUM_ATTACHMENTS, $del_where);  //delete attachment entries from table
    }
